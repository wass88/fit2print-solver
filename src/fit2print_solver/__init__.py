...
def collect_metrics(game):
    ...
    pieces_cnt = {c: sum(1 for p in game.pieces if p.kind == c) for c in ["R", "G", "B", "P", "A", "T"]}
    return {
...
def get_piece_subgame(game):
    ...
    others = [(i, p.height * p.width) for i, p in enumerate(game.pieces) if not is_forced(p)]
...
def calc_subsizes(total, areas, use = 4):
    ind_areas = list(sorted(((i, a) for i, a in enumerate(areas)), key=lambda x: -x[1]))
    res = []
    prev = [([], 0)] # (index, total)
...
def test_collect_subsizes():
    ...
    use = []
    for sub in subs:
        if not (expect_score(sub) >= 14): continue
        use.append(sub)
    return use
...
def parse_result(game, lines):
    ...
    def print_board():
        ...
        colors = [[game.pieces[c-1].kind if c is not None and c > 0 else "." for c in row] for row in table]
        print_table(colors)
    print_key("vplace")
    print_key("vcount")
    print_key("vpcell")
    print(peek_line(lines, "v_sphoto"))
    print_table(peek_table(lines, "vadjust", len(game.pieces)+1, len(game.pieces)+1))
    print_board()
    return True
